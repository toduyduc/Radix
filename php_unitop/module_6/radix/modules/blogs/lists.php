...
$allBlogNumber = getRows("SELECT * FROM `blog`");// lấy số lượng bản ghi blog
...
// truy vấn blog
$listBlog = getData("SELECT title,view_count,thumbnail,description,blog.create_at,name,category_id,blog.id as blog_id FROM blog join blog_categories on blog.category_id=blog_categories.id ORDER BY blog.create_at DESC LIMIT $offset,$perPage");
?>
		<!-- Blogs Area -->
					...
					<div class="col-12">
						...
						<div class="pagination-main">
							<ul class="pagination">
                                ...
                                if($page>1){
                                    $prevPage = $page-1;
                                    echo '<li class="prev"><a href="'._WEB_HOST_ROOT.'/?module=blogs&action=lists&page='.$prevPage.'"><i class="fa fa-angle-double-left"></i></a></li>';
                                }
                                ?>
                                ...
                                ?>
								<li class="<?php echo ($index==$page)?'active':false; ?>"><a href="<?php echo _WEB_HOST_ROOT.'/?module=blogs&action=lists&page='.$index;?>"><?php echo $index; ?></a></li>
								<?php 
                                } 
                                if($page<$maxPage){
                                    $lastPage = $page+1;
                                    echo '<li class="next"><a href="'._WEB_HOST_ROOT.'/?module=blogs&action=lists&page='.$lastPage.'"><i class="fa fa-angle-double-right"></i></a></li>';
                                }
                                ?>
