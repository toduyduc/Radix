...
// xử lý phân trang
$allpagesNumber = getRows("SELECT * FROM `pages` $filter");// lấy số lượng bản ghi nhóm người dùng

//1. xác định được số lượng bản ghi trên 1 trang
$perPage = _PER_PAGE; // mỗi trang có 3 bản ghi

//2. tính số trang
$maxPage = ceil($allpagesNumber/$perPage); //hàm ceil: làm tròn lên
...
$listPages = getData("SELECT `pages`.id,`title`, `pages`.create_at,fullname,`users`.id as user_id  FROM `pages` JOIN users ON `pages`.user_id=`users`.id $filter ORDER BY `pages`.create_at DESC LIMIT $offset,$perPage");
      ...
      <div class="container-fluid">
        ...
        <form action="" method="get">
            <div class="row">
                ...
                <div class="col-6">
                    <input type="search" class="form-control" name="keyword" placeholder="Nhập tên trang..." value="<?php echo (!empty($keyword))?$keyword:false; ?>">
                </div>
                <div class="col-3">
        ...
        <table class="table table-bordered">
            ...
            <tbody>
                ...
                <tr>
                    ...
                    <td><a href="<?php echo getLinkAdmin("pages","edit",['id'=>$item['id']]); ?>"> <?php echo $item["title"] ?></a></td>
                    <td><a href="?<?php echo getLinkQueryString('user_id',$item['user_id']);?>"><?php echo (!empty($item['fullname']))?$item['fullname']:false; ?></a> <a class="btn btn-danger btn-sm" style="padding: 0 5px;" href="<?php echo getLinkAdmin("pages","duplicate",['id'=>$item['id']]); ?>"> Nhân bản</a></td>
        ...
        <nav aria-label="Page navigation example" class="d-flex justify-content-end">
            <ul class="pagination">
                <?php
                    if($page>1){
                        $prevPage = $page-1;
                        echo '<li class="page-item"><a class="page-link" href="?module=pages'.$queryString.'&page='.$prevPage.'">Trước</a></li>';
                    }
                ?>
                <?php 
                ...
                ?>
                <li class="page-item <?php echo ($index==$page)?'active':false; ?>"><a class="page-link" href="?module=pages<?php echo $queryString; ?>&page=<?php echo $index;?>"><?php echo $index; ?></a></li>
                <!-- thêm active vào class để tô màu cho ô chuyển trang -->
                <?php }?>
                <?php
                    if($page<$maxPage){
                        $lastPage = $page+1;
                        echo '<li class="page-item"><a class="page-link" href="?module=pages'.$queryString.'&page='.$lastPage.'">Sau</a></li>';
                    }
                ?>
