...
$allCateNumber = getRows("SELECT * FROM `blog_categories` $filter");
...
$listCate = getData("SELECT *,(SELECT COUNT(`blog`.id) FROM blog WHERE `blog`.category_id=`blog_categories`.id) AS blog_count FROM `blog_categories` $filter ORDER BY create_at DESC LIMIT $offset,$perPage");
                ...
                <table class="table table-bordered">
                    ...
                    <tbody>
                        ...
                        <tr>
                            ...
                            <td><a href="<?php echo getLinkAdmin("blog_categories","",['id'=>$item['id'],'view'=>'edit']); ?>"><?php echo $item['name']; ?></a> <a class="btn btn-danger btn-sm" style="padding: 0 5px;" href="<?php echo getLinkAdmin("blog_categories","duplicate",['id'=>$item['id']]); ?>"> Nhân bản</a> <a class="btn btn-success btn-sm" style="padding: 0 5px;" href="#" target="_blank"> Xem</a></td>
                            <td><?php echo $item['blog_count']; ?></td>
                ...
                <nav aria-label="Page navigation example" class="d-flex justify-content-end">
                    <ul class="pagination">
                        <?php
                            if($page>1){
                                $prevPage = $page-1;
                                echo '<li class="page-item"><a class="page-link" href="?module=blog_categories'.$queryString.'&page='.$prevPage.'">Trước</a></li>';
                            }
                        ?>
                        <?php 
                        ...
                        ?>
                        <li class="page-item <?php echo ($index==$page)?'active':false; ?>"><a class="page-link" href="?module=blog_categories<?php echo $queryString; ?>&page=<?php echo $index;?>"><?php echo $index; ?></a></li>
                        <!-- thêm active vào class để tô màu cho ô chuyển trang -->
                        <?php }?>
                        <?php
                            if($page<$maxPage){
                                $lastPage = $page+1;
                                echo '<li class="page-item"><a class="page-link" href="?module=blog_categories'.$queryString.'&page='.$lastPage.'">Sau</a></li>';
                            }
                        ?>
